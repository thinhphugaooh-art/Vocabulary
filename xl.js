...
try {
  ...
  if (savedData) vocabList = JSON.parse(savedData);let vocabList = [];
let currentVocabIndex = -1;
let autoNextTimeout = null;
let revealedIndices = [];
let remainingIndices = [];
...
if (toggleDictBtn && dictSidebar) {
  toggleDictBtn.addEventListener('click', () => {
    dictSidebar.classList.toggle('active');
  });
}

if (closeSidebarBtn && dictSidebar) {
