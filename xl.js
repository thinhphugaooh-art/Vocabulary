...
// Tự động load từ bộ nhớ trình duyệt nếu có
try {
...
// Elements Pop-up Modal
const cambridgeModal = document.getElementById('cambridge-modal');
const closeModalBtn = document.getElementById('close-modal-btn');
const cambridgeFrame = document.getElementById('cambridge-frame');
...
startStudyBtn.addEventListener('click', () => {
  if (vocabList.length === 0) {
    alert('Danh sách từ vựng trống! Vui lòng thêm từ hoặc nạp từ file máy tính.');
    return;
  }
...
// MỞ POP-UP CAMBRIDGE DICTIONARY
cambridgeBtn.addEventListener('click', () => {
  ...
  if (firstWord) {
    const targetUrl = `https://dictionary.cambridge.org/dictionary/english/${encodeURIComponent(firstWord)}`;
    cambridgeFrame.src = targetUrl;
    cambridgeModal.classList.remove('hidden');
  }
});

closeModalBtn.addEventListener('click', () => {
  cambridgeModal.classList.add('hidden');
  cambridgeFrame.src = "about:blank";
});

openTabBtn.addEventListener('click', () => {
  if (cambridgeFrame.src && cambridgeFrame.src !== "about:blank") {
    window.open(cambridgeFrame.src, '_blank');
  }
});
...
function generateHint() {
  ...
  let hintStr = '';
  for (let i = 0; i < targetText.length; i++) {
    hintStr += revealedIndices.includes(i) ? targetText[i] : '_';
  }
...
function checkAnswer() {
  ...
  if (!userAns) return showFeedback('wrong', '⚠️ Vui lòng nhập câu trả lời!');

  const validAnswers = rawTarget.split(/[,;]/).map(ans => ans.trim()).filter(ans => ans.length > 0);
  if (validAnswers.some(target => userAns === target)) {
    showFeedback('correct', '🎉 Chính xác!');
    flashcard.classList.add('flipped');
    clearTimeout(autoNextTimeout);
    autoNextTimeout = setTimeout(loadNextCard, 800);
  } else {
    showFeedback('wrong', '❌ Sai rồi, thử lại nhé!');
  }
}
...
function loadNextCard() {
  ...
  if (vocabList.length === 0) {
    cardFrontText.innerText = "Chưa có từ vựng!";
    cardBackText.innerText = "";
    currentVocabIndex = -1;
    return;
  }

  if (remainingIndices.length === 0) resetRemainingIndices();
...
function displayCardContent(item) {
  ...
  if (mode === 'en-vi') {
    ...
    answerInput.placeholder = "Nhập từ Tiếng Anh...";
  }
}
...
function renderVocabs() {
  ...
  if (vocabList.length === 0) {
    vocabListUI.innerHTML = '<li style="justify-content: center; color: var(--text-sub);">Chưa có từ vựng nào!</li>';
    return;
  }
  vocabList.forEach(item => {
    ...
    li.innerHTML = `
      <div class="word-info">
        ...
        <p style="margin: 2px 0 0 0; font-size: 0.85rem; color: var(--text-sub);">${escapeHtml(item.meaning)}</p>
      </div>
...
// LƯU FILE VỀ MÁY TÍNH (EXPORT FILE JSON)
document.getElementById('export-btn').addEventListener('click', () => {
  if (vocabList.length === 0) return alert('Chưa có từ vựng để lưu!');
  const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(vocabList, null, 2));
  const downloadAnchor = document.createElement('a');
  downloadAnchor.setAttribute("href", dataStr);
  downloadAnchor.setAttribute("download", "TuVung_Backup.json");
  document.body.appendChild(downloadAnchor);
  downloadAnchor.click();
  downloadAnchor.remove();
});

// NẠP FILE TỪ MÁY TÍNH LÊN WEB (IMPORT FILE JSON)
...
fileInput.addEventListener('change', (e) => {
  ...
  reader.onload = (event) => {
    try {
      ...
      renderVocabs();
      resetRemainingIndices();
      alert('Đã nạp file từ vựng từ máy tính thành công!');
    } catch (err) {
